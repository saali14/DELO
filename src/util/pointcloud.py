...
def nested_list_to_tensor(pcds, num_points=10000, device='cuda'):
    return torch.stack([torch.tensor(random_downsampling(x[0], num_points=num_points), device=device) for x in pcds])
